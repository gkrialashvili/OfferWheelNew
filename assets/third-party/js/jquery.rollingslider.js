...
(function ($, window, document, undefined) {
  ...
  $.fn.RollingSlider = function (arg) {
    ...
    function animation() {
      if (direction == "next") {
        var prevActive = $showArea_li[num];
        num++;
        if (num > 2) num = 0;
        var activeCont = $showArea_li[num];
        $(prevActive).removeClass("active");
        $(activeCont).addClass("active");
        //点击next
        for (i = 0; i < column; i++) {
      ...
      if ($(".pos3").hasClass("active")) {
        $(".slider-icons").css("background-image", "url(images/3x/clover.png)");
      } 
      else  if ($(".pos2").hasClass("active")) {
        $(".slider-icons").css("background-image", "url(images/3x/cosmo.png)");
      }
      else  if ($(".pos4").hasClass("active")) {
        $(".slider-icons").css("background-image", "url(images/3x/book.png)");
      }
    }
    // 按需加载
